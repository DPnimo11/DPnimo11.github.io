...
Status: ready for generation and implementation  
...
physically thick technical panels. Match the north star's perspective, depth,
and hierarchy without making any panel busy or glossy. The final stack remains
decorative, static-site compatible, and legible in both themes.
...
- Panels overlap substantially and expose side thickness; they should not read
  as five unrelated flat cards.
...
- [ ] Inspect mockup 1 immediately before generation and load it as a
  composition/perspective reference.
- [ ] Generate and approve one blank Orientation B master slab first.
- [ ] Generate the terminal independently in Orientation A.
- [ ] Populate protein, map, EDA, and chessboard variants from the same
  Orientation B master while preserving its silhouette and camera.
- [ ] Generate one seven-piece chess sprite sheet with exactly the required
  white and black pieces.
- [ ] Budget three or four focused attempts per difficult asset rather than
  changing several prompt variables at once.
- [ ] Save every submitted prompt and decision in the run log.
- [ ] Store accepted source and alpha-processed assets under versioned names;
...
- [ ] Remove the flat chroma key with the installed imagegen helper.
- [ ] Validate alpha channels, transparent corners, opaque panel interiors,
  clean edges, and the absence of green spill.
- [ ] Crop the approved sprite sheet into seven consistently padded assets.
- [ ] Confirm every sprite has the same base anchor convention and camera.
- [ ] Optimize final files for hero use without blurring ink lines.
...
- [ ] Update `HeroArtifactStack.astro` to use terminal, protein, map, data, and
  chess layers in the approved order.
- [ ] Replace the current VM layer and asset references with the map layer.
- [ ] Place wrappers with `left` and `top` percentages; reserve transforms for
  drift and small final alignment corrections.
- [ ] Preserve alpha-masked broad panel shadows and tune them against the new
  silhouettes.
- [ ] Add deterministic terminal text only after the generated terminal slab
  matches the north star.
- [ ] Measure chessboard basis vectors and place the seven sprites at the exact
  FEN squares, anchored by the center of each base.
- [ ] Keep chess piece scale constant. Use z-order, bases, and contact shadows
  for depth.
- [ ] Verify that the f6/g6/g7 white cluster remains distinguishable.
...
- [ ] Blink the terminal block cursor.
- [ ] Add sparse map pings without changing either radius circle.
- [ ] Animate `Kf6-f5` once on chess hover or keyboard focus, including its
  contact shadow.
- [ ] Keep protein and EDA panels static.
- [ ] Retain only 2-3 pixel out-of-phase panel drift; omit scroll parallax in
  the first pass.
- [ ] Disable all motion under `prefers-reduced-motion: reduce`.
...
- [ ] Run `npm run build` after Astro or CSS implementation changes.
- [ ] Update `AGENTS.md` with the final assets and implementation behavior.
